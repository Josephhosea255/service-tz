...
export default function Favorites() {
  ...
  useEffect(() => {
    document.title = (lang === "sw" ? "Vipendwa" : "Favorites") + " — ServiceLink Tanzania";
    if (!user) return;
  ...
  return (
    <div className="container py-8">
      <h1 className="mb-6 text-2xl font-bold text-foreground md:text-3xl">{t("fav.title")}</h1>
      {items.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border p-12 text-center text-muted-foreground">{t("fav.empty")}</div>
      ) : (
