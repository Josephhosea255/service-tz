...
export default function Auth() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (mode === "signup") {
      ...
      else { toast.success("Account created — check your email if confirmation is required"); navigate("/dashboard"); }
    } else {
  ...
  return (
    <div className="container flex min-h-[calc(100vh-4rem)] items-center justify-center py-10">
      <Card className="w-full max-w-md p-6 shadow-elevated md:p-8">
        <h1 className="text-2xl font-bold text-foreground">{mode === "signin" ? "Welcome back" : "Create your account"}</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {mode === "signin" ? "Sign in to manage your listings & favorites." : "Join ServiceLink Tanzania to list your services."}
        </p>
        ...
        <p className="mt-4 text-center text-sm text-muted-foreground">
          {mode === "signin" ? "New to ServiceLink? " : "Already have an account? "}
          <button onClick={() => setMode(mode === "signin" ? "signup" : "signin")} className="font-medium text-primary hover:underline">
            {mode === "signin" ? "Create account" : "Sign in"}
          </button>
        </p>
        <p className="mt-2 text-center text-xs text-muted-foreground">
          Or just <Link to="/search" className="text-primary hover:underline">browse as guest</Link>.
        </p>
      </Card>
    </div>
  );
