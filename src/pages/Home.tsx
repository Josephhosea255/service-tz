    ...
    <div>
        ...
        <div className="container relative z-10 py-16 md:py-24">
          <div className="max-w-2xl space-y-5 text-primary-foreground">
            ...
            <h1 className="text-3xl font-bold leading-tight sm:text-4xl md:text-5xl">
              {t("home.headline.a")} <span className="text-secondary">{t("home.headline.b")}</span> {t("home.headline.c")}
            </h1>
            ...
            <form onSubmit={onSearch} className="mt-2 flex flex-col gap-2 rounded-2xl bg-background p-2 shadow-elevated sm:flex-row">
              ...
              <div className="hidden items-center gap-1 px-3 text-sm text-muted-foreground sm:flex">
                <MapPin className="h-4 w-4" /> {t("home.searchCity")}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <Button asChild variant="secondary" size="lg" className="font-semibold">
                <Link to="/dashboard"><PlusCircle className="mr-2 h-5 w-5" /> {t("home.postFree")}</Link>
              </Button>
              <Button asChild variant="ghost" size="lg" className="text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground">
                <Link to="/search">{t("home.browse")}</Link>
              </Button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 pt-6 sm:max-w-lg">
              <div className="rounded-xl bg-primary-foreground/10 p-3 backdrop-blur">
                ...
                <div className="text-xs text-primary-foreground/80">{t("trust.verified")}</div>
              </div>
              <div className="rounded-xl bg-primary-foreground/10 p-3 backdrop-blur">
                ...
                <div className="text-xs text-primary-foreground/80">{t("trust.rating")}</div>
              </div>
              <div className="rounded-xl bg-primary-foreground/10 p-3 backdrop-blur">
                ...
                <div className="text-xs text-primary-foreground/80">{t("trust.customers")}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="container py-10 md:py-14">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h2 className="text-xl font-bold text-foreground md:text-2xl">{t("home.cat.title")}</h2>
            <p className="text-sm text-muted-foreground">{t("home.cat.sub")}</p>
          </div>
        </div>
        <CategoryGrid />
      </section>
      ...
      <section className="bg-muted/40 py-12 md:py-16">
        <div className="container">
          <div className="mb-8 text-center">
            <h2 className="text-2xl font-bold text-foreground md:text-3xl">{t("how.title")}</h2>
            <p className="mt-2 text-sm text-muted-foreground">{t("how.sub")}</p>
          </div>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {[
              { icon: SearchIcon, step: "1", title: t("how.s1.title"), desc: t("how.s1.desc") },
              { icon: MessageCircle, step: "2", title: t("how.s2.title"), desc: t("how.s2.desc") },
              { icon: Zap, step: "3", title: t("how.s3.title"), desc: t("how.s3.desc") },
            ].map((s) => (
      ...
      <section className="container py-12 md:py-16">
        <div className="mb-6 flex items-end justify-between">
          <div>
            <h2 className="text-xl font-bold text-foreground md:text-2xl">{t("featured.title")}</h2>
            <p className="text-sm text-muted-foreground">{t("featured.sub")}</p>
          </div>
          <Button asChild variant="ghost" size="sm"><Link to="/search">{t("featured.viewAll")}</Link></Button>
        </div>
        {featured.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border p-10 text-center text-muted-foreground">
            {t("featured.empty.a")} <Link to="/dashboard" className="text-primary hover:underline">{t("featured.empty.b")}</Link>
          </div>
        ) : (
          ...
          <div className="relative z-10 flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
            <div className="max-w-xl space-y-3 text-primary-foreground">
              <div className="inline-flex items-center gap-2 rounded-full bg-primary-foreground/15 px-3 py-1 text-xs font-medium backdrop-blur">
                <ShieldCheck className="h-3 w-3" /> {t("providerCta.badge")}
              </div>
              <h2 className="text-2xl font-bold md:text-3xl">{t("providerCta.title")}</h2>
            ...
            <div className="flex flex-wrap gap-3">
              <Button asChild variant="secondary" size="lg" className="font-semibold">
                <Link to="/dashboard"><PlusCircle className="mr-2 h-5 w-5" /> {t("providerCta.post")}</Link>
              </Button>
              <Button asChild size="lg" variant="ghost" className="text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground">
                <Link to="/auth"><Phone className="mr-2 h-5 w-5" /> {t("providerCta.signup")}</Link>
              </Button>
            </div>
          </div>
